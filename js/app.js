...
import * as state from './modules/state.js';
        ...
        function setupEventListeners() {
            ...
            diagramTypeSelector.addEventListener('change', (e) => {
                ...
                if (hasObjects && state.diagramType !== newDiagramType) {
                    ...
                    if (!confirmed) {
                        ...
                        diagramTypeSelector.value = state.diagramType;
                        return;
                    }

                    // Clear the canvas and state arrays
                    canvas.clear();
                    ...
                    state.setEntities([]);
                    state.setErRelationships([]);
                    state.setTasks([]);
                    state.setMilestones([]);
                    state.setJourneySteps([]);
                    state.setJourneySections([]);
                }

                // Update the diagram type and tool palette
                state.setDiagramType(newDiagramType);
                updateToolPalette(newDiagramType);
                generateAndRenderMermaid();
            });
        ...
        function loadFromMermaidCode(code) {
            ...
            updateToolPalette(state.diagramType);
        }
        ...
        function getProjectData() {
            return {
                ...
                flowchartDirection: state.flowchartDirection,
                canvas: {
                ...
                actors: state.actors.map(a => ({ id: a.id, name: a.name })),
        ...
        function loadProjectData(projectData) {
            try {
                ...
                state.setDiagramType(projectData.diagramType || 'flowchart');
                state.setFlowchartDirection(projectData.flowchartDirection || 'TD');
                ...
                canvas.loadFromJSON(projectData.canvas.objects, () => {
                    ...
                    if (projectData.connections) {
                        state.setConnections(projectData.connections);
                    ...
                    if (projectData.actors) {
                        state.setActors(projectData.actors.map(a => ({
                            ...a,
                    ...
                    if (projectData.messages) {
                        state.setMessages(projectData.messages);
                    }

                    if (projectData.classes) {
                        state.setClasses(projectData.classes.map(c => ({
                            ...c,
                    ...
                    if (projectData.relationships) {
                        state.setRelationships(projectData.relationships);
                    }
